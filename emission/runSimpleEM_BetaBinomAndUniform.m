...
addParamValue(p,     'maxIter',        5e2, @isscalar);
...
if ~simpleFlag
    ...
    while (abs(dLambda) > p.Results.errTol) && ii< p.Results.maxIter
        ...
        [BB, pU] = mixBetaBinomUniform(q,r,N, theta);
        pBB = sum( bsxfun(@times, BB, Pf), 2);
        ...
        fh_dLdTheta = @(th)nansum( contr.*gi1.* ...
        ...
        [thetaEst, ~, ~] = newtonraphson(@(th)fh_dLdTheta(th), theta, options);
    ...
    fh_dLdTheta = @(th)nansum( contr.* ...
        nansum(bsxfun(@times, ddTheta_LogBetaBinomialThetaMu0(q, r, f, th), Pf), 2) , 1);
    
    [theta, ~, ~] = newtonraphson(@(th)fh_dLdTheta(th), theta, options);
end

if flagNegative && theta > 10
   warning('runSimpleEM_BetaBinomAndUniform:BadConvergence',...
       'bad convergence!') 
end

if ii == p.Results.maxIter
    warning('runSimpleEM_BetaBinomAndUniform:UnConverged',...
        'Convergence problem in lambda1! Cut on %u iterations', p.Results.maxIter);
end
