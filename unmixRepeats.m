...
for ii = modeNum:-1:1
    distribObj0 = gmdistribution.fit(XX , ii, 'Replicates', repeateEM);
...
PrTrue = mvnpdf(XX, mixtObj.mu(iTrue,:), mixtObj.Sigma(:,:,iTrue));
...
% figure; hist(Contr)
obj.contrib = Contr./max(Contr);% B_CONTR + (1 - B_CONTR)./(1 + C_CONTR./Odds);
...
if isempty(obj.notaRepeat)
    obj.notaRepeat = true(numel(logX1),1) ;
end
...
if strcmpi(p.Results.plotFlag, 'plot')
    ...
    scatter( logX1,  logX2, 3,  obj.contrib);
    xLim = get(gca, 'xlim');
    yLim = get(gca, 'ylim');
    nPoints = 50;
    ...
    %---------------------    
    if  flagRepeats
        [yhR, xhR] = hist( logX1(obj.notaRepeat), xh);
        b(2) = barstairs( xhR, yhR./sum(yh), [0.1, 0.9, .3], 'faceColor', 'none', 'linewidth', 2);
        %---------------------
        [yhN, xhN] = hist( logX1(~obj.notaRepeat),  xh);
    ...
    dx = diff(xh(2:3));
    xh = 0:dx/5:max(xh);
    logP(:,1) = distrMissMappedReads( 10.^xh', p.Results.READ_LENGTH, NUMBER_OF_SNPS_PER_READ) .* mixtObj.PComponents(iFalse);
...
>>>>>>> e8f845c502e49ef06f8aaf384e4c9c822e2e47d4
