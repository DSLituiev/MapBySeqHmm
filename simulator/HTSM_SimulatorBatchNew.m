...
for ii = par.numSeq:-1:1
    ...
    logPobsDy(:,ii) = AR.xPsel;
    fprintf('F-B algorithm took\t%2.2g\ts\n', toc)
end
%% == plot
[~, maxind] = max(logPobsDy, [], 1);
...
plot(mean(tSampling(maxind)), max(logPobsDy(:)), 'm^')

xlim([0, par.T_max])
ylim([ floor(min(logPobsDy(~isinf(logPobsDy)))), ceil(max(logPobsDy))] )

figure
bar(tSampling, sum( bsxfun(@eq, maxind', 1:par.SamplingPoints), 1) , 10)


figure
bar(tSampling, sum( 10.^-abs(bsxfun(@minus, tSampling', tSampling)), 1) , 10)
